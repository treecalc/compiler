	...
	public static String getConstNameInMethod(V constant) {
		if (constant instanceof VDouble) {
			if (constant.isInteger()) {
				return "_i" + constant.toString(); 
			} else {
				return "_d" + constant.toString().replace('.', '_').replace("E-", "E_");
			} 
			...
			if (!matcher.find()) {
				return "_s" + (s.length()==0 ? "_empty" : s);
			} else {
				return null;
			}
		} else if (constant instanceof VFuncref) {
			return "_funcref" + constant.funcrefValue();
		} else if (constant instanceof VTabref) {
			return "_tabref" + constant.tabrefValue();
		} else {
			return null;
		}
	}
...
//		op.put(LOGICAL_A.ND             , "and"     ); need extra care
		...
		op.put(COMPARE_BIGGER          , "big"     ); // >     compare numerical greater
		op.put(COMPARE_LESSEQUAL       , "smleq"   ); // <=    compare numerical less equal
		op.put(COMPARE_BIGGEREQUAL     , "bigeq"   ); // >=    compare numerical greater equal
	...
	static {
		...
		bf.put("ATAN2"                         ,new BFInfo("bf_atan2", 2, 2, false));
		...
		bf.put("V_STRING"                      ,new BFInfo("bf_v_string", 2, 2, true));
...
//special		"message"
	...
	private JavaFormula(PrintStream out, TcAst astFormula, ModelSimple model, int indent, String prefix, String postfix) {
		this.out = out;
	...
	private void visit(TcAst node) {
		...
		case KEYWORD_CASE: {
			...
			out.println(" = null; //will hold result of case"); indent();
		...
		case TT_INPUTCALCCALLSIMPLE: {
			...
			if (autocounters!=null) {
				addVar(node, "I." + inputname + "$" + propname + "(_s, (V[]) null)");
			} else {
				addVar(node, "I." + inputname + "$" + propname + "(_s)");
			}
...
//		}
		...
		case TT_INPUTACCESSRAWSELF: {
			...
			int inputindex = input.getIndex();
			addVar(node, "_s.getInput(" + inputindex + "/* " + name + "*/" + ", _index)");
			break;
		}
		case TT_INPUTACCESS: {
		...
		case KEYWORD_TABROWS: {
			...
			if (tab.getType()==ID) {
				...
				String sc = getConstantFormula(
						"V.getInstance(" + table.getNumrows() + ")",
						V.getInstance(table.getNumrows())
						);
				addVar(node, sc);
			} else {
				...
				String s = "((V) Tables.dynamicCall("
						+ getVar(tab) + ", Tables.CALL_NUMROWS))";
				addVar(node, s);
			}
			break;
		}
		...
		case KEYWORD_CELLX: {
			...
			if (tab.getType()==ID) {
				...
				for (int i=0; i<rows.getChildCount(); i++) {
					...
					printVarname(rows.getChild(i), s);
				}
				for (int i=0; i<cols.getChildCount(); i++) {
					s.append(", ");
					printVarname(cols.getChild(i), s);
				}
				s.append(")");
				addVar(node, s.toString());
			} else {
				...
				switch(method) {
				case 11: callconst = "CALL_CELL_S_S"; break;
				case 12: callconst = "CALL_CELL_S_R"; break;
				case 21: callconst = "CALL_CELL_R_S"; break;
				case 22: callconst = "CALL_CELL_R_R"; break;
		...
		case KEYWORD_EXISTS: {
			...
			if (tab.getType()==ID) {
				...
				s.append("V.getInstance(");
				s.append(tablename);
				s.append(".findRowExact(");
				for (int startind=1, i=startind; i<node.getChildCount(); i++) {
					if (i>startind) {
						s.append(", ");
					}
				...
				StringBuffer s = new StringBuffer(256);
				s.append("V.getInstance((Integer)Tables.dynamicCall(");
				printVarname(tab, s);
				s.append(", ");
				s.append("Tables.CALL_EXIST");
				for (int i=1; i<node.getChildCount(); i++) {
					s.append(", ");
					s.append(getVar(node.getChild(i)));
				}
		...
		case KEYWORD_LOOKDOWNX:	{
			...
			if (tab.getType()==ID) {
				...
				if (node.getType()==KEYWORD_LOOKUPX) {
					s.append(".findIntervalUpColumn(");
				} else {
					s.append(".findIntervalDownColumn(");
				}
				...
				s.append(", ");
				if (node.getType()==KEYWORD_LOOKUPX) {
					s.append("Tables.CALL_INTERVAL_UP_COL");
				} else {
					s.append("Tables.CALL_INTERVAL_DOWN_COL");
				}
				s.append(", ");
				s.append(getVar(node.getChild(node.getChildCount()-1)));
		...
		case TT_DYNTABLE: {
			...
			StringBuffer s = new StringBuffer(256);
			s.append("((V) Tables.dynamicCall(");
			s.append(getVar(dyntableformula));
			s.append(", ");
			if (columnaccess==null) {
				s.append("Tables.CALL_EXACT"); 				
			} else {
				s.append("Tables.CALL_EXACT_COL, ");
				s.append(getVar(columnaccess));
			}
			for (int i=0; i<index.getChildCount(); i++) {
				s.append(", ");
...
//			Scope scope = id.getScope();
		...
		case KEYWORD_VECTORX: {
			...
			out.print("ArrayList<V> ");
			out.print(sumvar);
			out.print(" = new ArrayList<V>(");
			out.print("(int)(");
			out.print(vardoubleto);
			out.print('-');
			out.print(vardoublefrom);
...
//			addVar(node, "vTabrefValue(getInstance(\"" + name + "\"))");
	...
	private void printif(TcAst node, TcAst cond, TcAst thenformula,	TcAst elseformula) {
		addVar(node);
		out.print("V "); printVarname(node); out.println(';'); indent();
		visit(cond);
		out.print("if (");
		printVarname(cond);
	...
	public void or(TcAst node) {
		...
		indent++; indent();
		printVarname(node); 
		out.println(" = V.getInstance(true);"); 
		indent--; indent();
		out.println("}"); indent();
	}
	...
	public static PrintFormulaInfos printFormula(PrintStream out, TcAst astFormula, ModelSimple model, int indent, String prefix, String postfix) {
		JavaFormula o = new JavaFormula(out, astFormula, model, indent, prefix, postfix);
		o.run();
		return o.printFormulaInfos;
	}
	public static boolean isSimple(ModelSimple model, TcAst node) {
//		return true;
		return new JavaFormula((PrintStream)null, node, model,0, "", "").isSimple(node);
	}
...
//			case LIST: return true; //because the property is cached itself
//			}
//			return false;
		}
		case KEYWORD_SUMX:
